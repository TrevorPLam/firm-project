 ...
 */
const IPV6_PATTERN = /^(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}$|^::(?:[0-9a-fA-F]{1,4}:){0,6}[0-9a-fA-F]{1,4}$|^(?:[0-9a-fA-F]{1,4}:){1,7}:$|^(?:[0-9a-fA-F]{1,4}:){0,6}::$/;
 ...
 */
function createFingerprint(headers: { get: (name: string) => string | null }): string {
 ...
 * ```
 */
export function getClientIdentifier(headers: { get: (name: string) => string | null }): string {
  ...
  console.warn('[ip-utils] No valid IP found in headers, using fingerprint fallback', {
