...
async function migrateBlogPosts() {
  ...
  for (const post of posts) {
    try {
      const fullPost = (await import("../app/lib/blog-data")).getPostBySlug(
        post.slug,
      );
...
async function migrateCaseStudies() {
  ...
  for (const study of studies) {
    try {
      const fullStudy = (
        await import("../app/lib/portfolio-data")
      ).getCaseStudyBySlug(study.slug);
...
async function main() {
  ...
  try {
    ...
    // Summary
    const totalSuccess =
      blogResults.successCount + caseStudyResults.successCount;
