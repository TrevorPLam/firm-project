...
console.log(
  "2. If intlMiddleware returns a response (redirect), code returns early",
);
console.log(
  "3. The early return path sets CSP header but NEVER sets x-nonce on request",
);
console.log(
  "4. x-nonce header is only set AFTER the early return (lines 44-45)",
);
console.log(
  "5. Server Components call getNonce() which reads x-nonce from headers",
);
console.log(
  "6. Result: getNonce() returns empty string on i18n redirect paths\n",
);
...
console.log("  }");
console.log(
  '  requestHeaders.set("x-nonce", nonce)  // Only reached if no redirect',
);
console.log("\n");
...
console.log(
  "- Or merge x-nonce into the intlResponse request before returning",
);
console.log("- Ensure CSP header is set on the final response");
console.log("\n");

console.log(
  "Verification complete. See docs/security.md for implementation fix.",
);
