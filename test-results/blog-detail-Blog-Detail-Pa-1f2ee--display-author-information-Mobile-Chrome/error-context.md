...
- Name: blog-detail.spec.ts >> Blog Detail Page >> should display author information
- Location: app\e2e\blog-detail.spec.ts:21:7
...
Locator: getByText('David Park')
Expected: visible
Error: strict mode violation: getByText('David Park') resolved to 3 elements:
    1) <div class="text-foreground font-semibold">David Park</div> aka locator('div').filter({ hasText: /^David Park$/ })
    2) <h3 class="mb-1 text-xl font-bold">David Park</h3> aka getByRole('heading', { name: 'David Park' })
    3) <p class="text-foreground/70 mb-4">…</p> aka getByText('David Park is part of the')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('David Park')
