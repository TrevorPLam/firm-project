...
Error: locator.click: Error: strict mode violation: getByRole('link', { name: 'Services' }) resolved to 2 elements:
    1) <a href="/en/services" class="border-foreground/20 hover:border-primary hover:text-primary rounded-full border-2 px-8 py-4 text-center text-lg font-semibold transition-all">Explore Services</a> aka getByRole('link', { name: 'Explore Services' })
    2) <a href="/en/services" class="text-foreground/60 hover:text-foreground transition-colors">Services</a> aka getByRole('link', { name: 'Services', exact: true })
